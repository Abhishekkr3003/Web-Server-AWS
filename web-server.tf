...
resource "aws_s3_bucket" "bucket_for_image" {
  ...
  provisioner "local-exec" {
     command = "aws s3 cp /home/abhishek/terraform/test/html5-practice/IMG_20200314_191513.jpg  s3://${aws_s3_bucket.bucket_for_image.bucket}/image2.jpg --acl public-read"
  }
  depends_on = [
...
resource "null_resource" "while_destroy" {
  provisioner "local-exec" {
    when = destroy
     command = "rm -rf /home/abhishek/terraform/test/html5-practice"
  }
  provisioner "local-exec" {
    when = destroy
